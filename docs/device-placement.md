...
| `concat(axis=1)`       | impl | impl (round-trip) | Last-axis 2D concat, done in host memory. GPU inputs are downloaded, concatenated, and the result uploaded — acceptable for small MHA use; a fused kernel can replace this later. Backward slices upstream grad back into each input. |
| `MultiHeadAttention`   | impl (composed) | impl (composed) | Per-head `Linear` projections + per-head SDPA + `concat` + output `Linear`. Optional attention `Dropout`. |
...
| `LearnedPositionalEmbedding` | impl | impl | Trainable `[maxLen, embedDim]` table wrapped as an `Embedding` submodule; positions `0..N-1` gathered via existing embedding op, then added. |
...
| `GPT.generate`         | impl (host loop) | impl (host loop) | Autoregressive sampler; runs Dart-side around per-step `call()`. Greedy / temperature / top-k. Sampling itself is CPU (softmax + CDF draw on a `List<double>`) — the forward pass respects the model's device. |
