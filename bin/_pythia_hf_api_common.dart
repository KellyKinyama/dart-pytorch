/// Shared CLI + HTTP API runner for HuggingFace Pythia checkpoints
/// loaded via `PythiaHFLoader`. Mirrors `_gpt2_hf_api_common.dart`
/// but builds a `PythiaModel` and reads the token→id map out of HF's
/// merged `tokenizer.json` (Pythia doesn't ship a plain vocab.json).
///
/// See `bin/pythia/run_gpu_api.dart` for a concrete use.
library;
...
Future<void> runPythiaApi({
  required String modelName,
  required String defaultPath,
  required PythiaConfig Function({required Device device}) configFactory,
...
_Loaded _buildAndLoad(
  _Options o,
  String modelName,
  PythiaConfig Function({required Device device}) configFactory,
) {
  final f = File(o.path);
  if (!f.existsSync()) {
  ...
  stdout.writeln(
    'Building Pythia ($modelName, $deviceLabel, embed=${cfg.embedDim}, '
    'layers=${cfg.numLayers}, heads=${cfg.numHeads})...',
  );
  final model = PythiaModel(cfg);

  stdout.writeln('Loading safetensors from ${o.path} ...');
  final t0 = DateTime.now();
  final report = PythiaHFLoader.loadFile(model, o.path);
