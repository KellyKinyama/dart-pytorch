...
class Lc0Input {
  ...
  static Tensor fromFen(String fen) {
    ...
    // Castling rights — LC0's convention is STM-K, STM-Q, opp-K, opp-Q.
    ...
    if (stmK) _fillPlane(data, 104, 1.0);
    if (stmQ) _fillPlane(data, 105, 1.0);
    if (oppK) _fillPlane(data, 106, 1.0);
    if (oppQ) _fillPlane(data, 107, 1.0);

    // Side-to-move plane: 0 = white to move, 1 = black to move.
    _fillPlane(data, 108, blackToMove ? 1.0 : 0.0);

    // Rule-50 counter normalized. LC0 divides by 99 or 100 depending
    // on version; use 99 to match the T-series nets.
    _fillPlane(data, 109, (rule50 / 99.0).clamp(0.0, 1.0));

    // Ply / total half-move counter — zero for demo (see docstring).
    _fillPlane(data, 110, ((fullMove - 1) * 2 + (blackToMove ? 1 : 0)) / 500.0);

    // Constant "all ones" plane 111.
